...
**Your assignment is to write an .Rmd notebook that estimates the selection coefficient for different variants, and/or for a province rather than all of Canada.**
...
This tells me the slope of my regression for the XBB* variant is `r round(coef(lm.XBB)[2],3)` and the intercept is `r round(coef(lm.XBB)[1],3)`. For the BQ* variant, the slope is `r round(coef(lm.BQ)[2],3)` and the intercept is `r round(coef(lm.BQ)[1],3)`. You can get these values to print out in your Rmarkdown by writing `round(coef(lm.BQ)[1],3)` but with an `r` followed by a space after the first `.
